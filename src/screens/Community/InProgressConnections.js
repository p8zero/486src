...
export default function InProgressConnections({navigation}) {
  ...
  useEffect(() => {
    ...
    .then(result => {
      ...
      setMatch(data.filter(match => match.status == 'pending'));
    ...
    <SafeAreaView
      ...
      <View style={{backgroundColor:'#FFF5EF',
        ...
        <View>
          <FlatList
            data={match}
            renderItem={({item}) => <PotentialConnection item = {item} navigation={navigation} detailsScreen={'InProgressConnectionDetails'} />}
