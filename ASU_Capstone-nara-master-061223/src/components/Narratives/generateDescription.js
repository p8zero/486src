...
const generateDescription = ({ inputData, flow, mainChar, otherChar }) => {

  let converted = inputData;
  if(inputData.length > 50){
    if (flow === "clarityInTheMoment") {
      const lengths = [8, 9, 16, 8, 8, 4, 16, 4, 4, 6, 14];
      converted = convertArray(inputData, lengths);
  ...
  let reff =
    flow == "clarityInTheFuture"
      ? clarInTheFut_SitMastFeatList
      : clarInTheMom_BehavMastFeatList;
  let desc = "";
  let count = 0;
  for (i = 0; i < converted.length; i++) {
    if (converted[i].includes(1)) {
      desc += "\n" + reff[i].narrativeElement + "\n\n";
      count = 0;
    }

    for (j = 0; j < converted[i].length; j++) {
      if (converted[i][j] == 1) {
        desc += reff[i].data[j].description + "\n\n";
        count++;
      }
    }
  }

  return desc.trimStart();
};
